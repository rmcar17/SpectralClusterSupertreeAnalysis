...
SUP = "SUP"
SCS = "SCS"
SCS1 = "SCS Depth Normalise"
SCS2 = "SCS No Normalise"
MCS = "MCS"
BCD = "BCD"

SCRIPTS = {  # Might want to go through options combinations here?
    SUP: "./run_sup.sh",
    SCS: "./run_scs.sh",
    MCS: "./run_mcs.sh",
    BCD: "./run_bcd.sh",
...
def run_methods(
    ...
    for method in methods:
        ...
        print(" ".join([SCRIPTS[method], source_tree_file, *OPTIONS.get(method)]))
        start_time = time.time()
        result = subprocess.run(
            [SCRIPTS[method], source_tree_file, *OPTIONS.get(method)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        end_time = time.time()
